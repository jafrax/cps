...
public class InputProductsController extends Window {

    private Logger log = LoggerFactory.getLogger(InputProductsController.class);
    ...
    private void initComponents() {
        ...
        satuan = (Textbox) getFellow("satuan");
    ...
    public void subKategoriSelected(){
        ...
        try {
        	 String query = "select a.nama,0 from "+Libs.getDbName()+".dbo.kategori a inner join "+Libs.getDbName()+".dbo.subkategori b "
        	 		+ "on a.kode=b.kd_kategori where b.kode = '"+sub+"' ";
              List<Object[]> l = s.createSQLQuery(query).list();
             for (Object[] o : l) {
            	 kategori.setText(Libs.nn(o[0]));
            }
        } catch (Exception ex) {
