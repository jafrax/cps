...
public class SalesOrderController extends Window {
    ...
    private void tampil(){
	 ...
	 try {
		  s.beginTransaction();
	        String query  = "select tgl_order,kd_cust, kategori from " + Libs.getDbName()+".dbo.salesorder where no_order = '"+noso.getValue()+"'  " ;
	        System.out.println(query);
	        List<Object[]> l = s.createSQLQuery(query).list();
	        for (Object[] o : l) {
    ...
    private void populateClients(int offset, int limit) {
    	 ...
    	 if (!val.isEmpty()) {
	        ...
	        try {
	            ...
	            String q1 = "select kode, nama ,perusahaan ";
	            String q2 = "from "+Libs.getDbName()+".dbo.customer  where kd_sales = '"+sc+"' ";
	            String q3 = "";
    ...
    public void productSelected() {
        if (lbProducts.getSelectedCount()>0) {
            ...
            for (int i = 0; i <= 6; i++){
            	String a = "New Harga"; 
            	if ( i == 1){ a = "Harga A";} else if(i == 2){ a = "Harga B";} else if(i == 3){ a = "Harga C";}
            	else if(i == 4){ a = "Harga D";}else if(i == 5){ a = "Harga E";}else if(i == 6){ a = "Harga F";};
	            	if( i == 0){
    ...
    private boolean kondisi() {
        ...
        if (Integer.parseInt(jumlah.getText().replaceAll("\\s+", " ")) >= sstok ) {
            ...
            valid = Libs.showErrorForm("Please jumlah jumlah!");
