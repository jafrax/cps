...
public class DaftarSalesOrderController extends Window {

    private Logger log = LoggerFactory.getLogger(DaftarSalesOrderController.class);
    private Listbox lb;
    ...
    private void initComponents() {
        ...
        ((Toolbarbutton) getFellow("check")).setDisabled(true);
    ...
    private void populate(int offset, int limit) {
        ...
        try {
            ...
            String q1 = "select no_order, tgl_order,tgl_tempo, kd_cust, kd_kategori, total,check_spv,kd_sales ";
            ...
            for (Object[] o : l) {
                ...
                int ch = Integer.parseInt(((String)Libs.nn(o[6])));
                String Check = "";
                if (ch == 0 ){ Check = "FALSE"; } else if (ch == 1 ){Check = "TRUE";}
                li.appendChild(new Listcell(Check));
    ...
    public void DetailSelected() {
    	 ((Toolbarbutton) getFellow("check")).setDisabled(false);
    ...
    public void check(){
    	...
    	if (Messagebox.show("Do you want to Check this Data ?", "Confirmation", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, Messagebox.CANCEL)==Messagebox.OK) {
 		ihm = (lb.getSelectedItem().getValue());    
 		Session s = Libs.sfDB.openSession();
 		try {
			s.beginTransaction();
		 	
		 	String sql= "update " + Libs.getDbName()+".dbo.salesorder  set check_spv = 1  where  no_order =  '"+ihm[0]+"' and flg=1 " ;
	        ...
	        s.getTransaction().commit();  
	         ...
	         log.error("delete", e);
	         Messagebox.show("Error status has been Check ", "Information", Messagebox.OK, Messagebox.INFORMATION);
	     } finally {
