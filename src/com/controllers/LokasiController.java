...
public class LokasiController extends Window {

    private Logger log = LoggerFactory.getLogger(LokasiController.class);
    private Listbox lb;
    private Paging pg;
    ...
    private void populate(int offset, int limit) {
        ...
        try {
            ...
            String q2 = "from "+Libs.getDbName()+".dbo.lokasi ";
            String q3 = "";
    ...
    public void Input() {
    	Window w = (Window) Executions.createComponents("views/InputLokasi.zul", this, null);
        w.doModal();
    }
    ...
    public void delete(){
    	...
    	if (Messagebox.show("Do you want to remove this Sales ?", "Confirmation", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, Messagebox.CANCEL)==Messagebox.OK) {
        	...
        	String sql = " delete "+Libs.getDbName()+".dbo.stokperlok where  "
                    +" kd_lokasi ='"+ Libs.nn(ihm[0]) + "' ";
        	
        	String qry = " delete "+Libs.getDbName()+".dbo.lokasi where  "
                    +" kode ='"+ Libs.nn(ihm[0]) + "' ";
            ...
            Messagebox.show(" Lokasi has been removed", "Information", Messagebox.OK, Messagebox.INFORMATION);
        	} catch (Exception ex) {
             log.error("delete", ex);
         } finally {
             s.close();
