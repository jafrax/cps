...
public class KategoriController extends Window {

    private Logger log = LoggerFactory.getLogger(KategoriController.class);
    private Listbox lb;
    private Paging pg;
    ...
    private void populate(int offset, int limit) {
        ...
        try {
            ...
            String q1 = "select "
                    + "kode,nama,awal_nota,tempobayar,tempokomisiE,tempokomisiA,tempokomisiB ";
            String q2 = "from "+Libs.getDbName()+".dbo.kategori ";
            String q3 = "";
            ...
            for (Object[] o : l) {
                ...
                A kategori = new A(Libs.nn(o[1]).trim());
                kategori.setStyle("color:#00bbee;text-decoration:none");
                Listcell cell = new Listcell();
                cell.appendChild(kategori);
    			li.appendChild(cell);
                ...
                kategori.addEventListener(Events.ON_CLICK, new EventListener<Event>() {

					@Override
					public void onEvent(Event arg0) throws Exception {
						showKategoriDetail(Id);
					}
				});
    ...
    public void showKategoriDetail(String Id){
    	Window w = (Window) Executions.createComponents("views/master/produk/EditKategori.zul", Libs.getRootWindow(), null);
    ...
    public void delete() {
        if (Messagebox.show("Do you want to remove this Kategori ?", "Confirmation", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, Messagebox.CANCEL)==Messagebox.OK) {
        	ihm = (lb.getSelectedItem().getValue());
        	
        	Session s = Libs.sfDB.openSession();
        	try{
        	s.beginTransaction();
        	String qry = " update "+Libs.getDbName()+".dbo.kategori set tdkpakai = 1  where  "
                    +" kode ='"+ Libs.nn(ihm[0]) + "' ";
            ...
            ((Toolbarbutton) getFellow("tbnDelete")).setDisabled(true);
            Messagebox.show(" Kategori has been removed", "Information", Messagebox.OK, Messagebox.INFORMATION);
        	} catch (Exception ex) {
             log.error("delete", ex);
         } finally {
             s.close();
