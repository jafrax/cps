package com.controllers.pembelian;
...
//        tipe = (Combobox) getFellow("tipe");
        ...
        discrp=(Textbox) getFellow("disc1");
        discpc=(Textbox) getFellow("disc2");
...
//        tipe.setSelectedIndex(0);
    ...
    private void populateProducts(int offset, int limit) {
       ...
       if (gudang.getText() == ""){
        ...
        try {
        	String q0 = "select count(*) ";
            String q1 = "select a.kode,a.namabrg,a.merkbrg,b.jumlah,a.tipebrg,a.ukuran,a.warna  ";
            String q2 = "from "+Libs.getDbName()+".dbo.barang a inner join "+Libs.getDbName()+".dbo.stokperlok b ON a.kode=b.kd_barang ";
            String q3 = " where b.kd_lokasi = '"+gudang.getText()+"'  and a.tdkpakai = 0 ";
            String q4 = "order by kode  asc ";

            
            if (whereProducts!=null) {
              q3 += "and (" + whereProducts + ") ";
            }
...
//            keterangan.setText(productPOJO.getProductName());
            namabarang.setText(productPOJO.getProductName());
    ...
    public void quickSearchProducts() {
        ...
        if (!tQuickSearchProducts.getText().isEmpty()) {
            whereProducts = "a.namabrg like '%" + tQuickSearchProducts.getText() + "%' or "
                    + "a.kode like '%" + tQuickSearchProducts.getText() + "%' ";
            populateProducts(0, pgProducts.getPageSize());
        } else {
            refreshProducts();
        }
    ...
    private void populate(int offset, int limit) {
        ...
        try {
            ...
            String q3 = "where a.no_fak='"+nofaktur.getText()+"' and leuser= '"+user+"'  ";
            System.out.println(q1 + q2 + q3);
            System.out.println(q0+q2+q3);
            Integer rc = (Integer) s.createSQLQuery(q0 + q2 + q3).uniqueResult();
            pg.setTotalSize(rc);
...
//             prk = prk.substring(prk.indexOf("(")+1, prk.indexOf(")"));
             ...
             int total = Integer.parseInt(harga.getText()) * Integer.parseInt(jumlah.getText()) ;
         	 ...
         	 String qry  = "insert into  " + Libs.getDbName()+".dbo.beli_rin "
         	 		+ "(no_fak,kd_barang,banyak,hrgperbrg,kodelok,disc_rp,disc_pc,leuser )  "
         	 		+ " values "
         	 		+ " ( '"+nofaktur.getText()+"', '"+bbProduct.getText()+"','"+jumlah.getText()+"',"
         	 		+ " '"+harga.getText()+"','"+gudang.getText()+"',"
